...
export default function NotificationSystem() {
  ...
  const showReturnNotification = () => {
    if (Notification.permission === 'granted') {
      ...
      // Use time-based selection instead of random for consistency
      const now = new Date();
      const titleIndex = now.getDate() % titles.length;
      const bodyIndex = now.getHours() % bodies.length;
      
      const selectedTitle = titles[titleIndex];
      const selectedBody = bodies[bodyIndex];

      const notification = new Notification(selectedTitle, {
        body: selectedBody,
        icon: '/icon-192x192.png',
        badge: '/badge-72x72.png',
  ...
  const addInAppNotification = (message: string, type = 'info') => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      (window as any).showNotification = addInAppNotification;
      (window as any).showNewsNotification = showNewsNotification;
      (window as any).requestNotificationPermission = requestPermission;
    }
  }, []);
