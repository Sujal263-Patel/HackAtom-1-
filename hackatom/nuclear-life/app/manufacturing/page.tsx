...
export default function ManufacturingPage() {
  ...
  const currentData = manufacturingData[selectedIsotope as keyof typeof manufacturingData];
  const currentCosts = currentData.costs[facilityType as keyof typeof currentData.costs];

  const calculateFinancials = () => {
    const initialInvestment = currentCosts.initial;
    const annualOperating = currentCosts.operating;
    const annualRevenue = currentData.revenue[facilityType as keyof typeof currentData.revenue];
            ...
            {Object.keys(manufacturingData).map((isotope) => (
              ...
              >
                <div className="font-bold text-lg mb-2">
                  {manufacturingData[isotope as keyof typeof manufacturingData].name}
                </div>
                <div className="text-gray-600 text-sm">
                  {manufacturingData[isotope as keyof typeof manufacturingData].description}
                </div>
              </button>
            ))}
          </div>
