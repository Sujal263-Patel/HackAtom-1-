...
type SectorKey = 'medicine' | 'agriculture' | 'industry' | 'space';

export default function SectorApplications() {
  const [selectedSector, setSelectedSector] = useState<SectorKey>('medicine');
  ...
  const currentSector = sectors[selectedSector as keyof typeof sectors];
      ...
      <div className="bg-white rounded-xl p-8 shadow-sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {Object.entries(sectors).map(([key, sector]) => (
            <button
              key={key}
              onClick={() => setSelectedSector(key as SectorKey)}
